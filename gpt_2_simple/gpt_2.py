...
def finetune(
    ...
    LOGGER.info("dataset has %s tokens", data_sampler.total_size)
    ...
    def generate_samples():
        ...
        with open(
            os.path.join(SAMPLE_DIR, run_name, "samples-{}").format(counter),
            "w",
        ) as fp:
            fp.write("\n".join(all_text))
    ...
    try:
        ...
        LOGGER.info("Interrupted job, saving before stopping")
        save()
...
def is_mounted():
    ...
    assert os.path.isdir(
        "/content/drive"
    ), "You must mount first using mount_gdrive()"
